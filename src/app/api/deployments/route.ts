...
const ALLOWED_MIME_TYPES = new Set([
    ...
    "application/octet-stream", // some clients send .zip as this
]);
...
function validateSubdomain(subdomain: string): { ok: boolean; error?: string } {
    ...
    if (!SUBDOMAIN_REGEX.test(s)) {
        return { ok: false, error: "Subdomain must be 3-63 characters, lowercase alphanumeric and hyphens only, and cannot start or end with a hyphen." };
    }
...
async function injectTrackingPixel(indexHtmlPath: string, deploymentId: string): Promise<void> {
    ...
    try {
        ...
        // Remove any previously injected pixel (idempotent on re-deploy)
        ...
        if ($("body").length) {
            ...
            // No <body> tag — append directly
            $.root().append(pixelTag);
        }
        await writeFile(indexHtmlPath, $.html(), "utf-8");
    } catch {
        // Non-fatal — pixel injection failure shouldn't block deployment
...
async function extractZip(buffer: Buffer, targetDir: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
        ...
        stream.on("entry", (entry: unzipper.Entry) => {
            ...
            // Normalize: strip leading component if all files are inside a single folder
            const entryPath = entry.path.replace(/\\/g, "/");
            // Block path traversal inside ZIP
...
// ─── GET /api/deployments ─────────────────────────────────────────────────────
// Student: list own deployments + user limits
...
// Student: deploy a new site (HTML or ZIP)
...
export async function POST(req: NextRequest) {
    ...
    try {
        // 1. Load user deployment settings
        ...
        // 5. Check subdomain uniqueness in DB
        ...
        // 7. Set up target directory
        const targetDir = safePath(STUDENT_SITES_BASE, rawSubdomain);
        ...
        if (isHtml) {
            ...
            // Ensure index.html exists after extraction
        ...
        // 9. Create DB record first (we need the ID for pixel injection)
